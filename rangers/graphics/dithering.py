...
def generate_matrix(size: int) -> list[list[float]]:
    ...
    result: list[list[float]] = [[0 for x in range(2 ** size)] for y in range(2 ** size)]
    ...
    for i in range(2 ** size):
        for j in range(2 ** size):
            ...
            result[i][j] = ((small_matrix[i % k][j % k] * (k ** 2) - 1) * 4 + a + 1) / (
                (2 ** size) ** 2
            )
...
def dither_bayer(img: ImageType, *, bit_trunc: int = 5, matrix_n: int = 3) -> ImageType:
...
def dither_random(img: ImageType, *, bit_trunc: int = 5) -> ImageType:
...
def dither_error_diff(img: ImageType, *, bit_trunc: int = 20) -> ImageType:
    # assert 0 <= bit_trunc <= 8, f'Invalid value: bit_trunc={bit_trunc}'
